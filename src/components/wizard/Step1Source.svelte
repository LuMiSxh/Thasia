<script lang="ts">
  ...
  async function handleNext() {
    if (!wizard.sourcePath) { error = 'Please select a source.'; return; }
    loading = true;
    error = '';
    try {
...
<div>
  <input type="text" readonly value={wizard.sourcePath} placeholder="No source selected" style="width:100%;padding:8px;margin-bottom:8px;" />
