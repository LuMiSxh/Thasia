use crate::{
    ...
    models::{ConversionEvent, ConversionOutput, ConvertOptions},
};
...
pub(crate) async fn convert_volume(
    ...
    on_event: &(dyn Fn(ConversionEvent) + Send + Sync),
) -> AppResult<VolumeConversionResult> {
    ...
    let output_path = pkg.output_path().ok_or(AppError::MissingOutputPath)?;
    ...
    while let Some(result) = rx_processed.recv().await {
        ...
        current += 1;
        stats.add_image(&img);
        emit_image_progress(on_event, vol_num, current, total, started.elapsed(), &stats);
    ...
    if current != total {
        return Err(AppError::IncompleteVolume {
            volume_num: vol_num,
            expected: total,
            actual: current,
        });
...
fn emit_image_progress(
    ...
    on_event(ConversionEvent::ImageProgress {
        volume_num,
        current,
        total,
